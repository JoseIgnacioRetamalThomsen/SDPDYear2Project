...
public class SpanishPlainText extends Archive implements Displayable
{
	...
	// 0 : number of lines, 1: extra characters count (numbers,$,etc) , 2 total , 3 accents
	// letters //á í é ñ ú ó
	final char CHARACTERS[] =
	{ '0', 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n','ñ', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v',
			'w', 'x', 'y', 'z', ' ', '1', '2','3' };//total 32
		...
		while (scanner.hasNext())
		{
			...
			for (int i = 0; i < temporalLine.length(); i++)
			{
				...
				while (!isFound && position < this.CHARACTERS.length - 3)
				{
					...
					else if(temporalChar =='á')//add for extra leters with accent
					{
						this.charactersCount[1]++;//add to a
						this.charactersCount[31]++;//add to accents
					}
					else if(temporalChar =='é')//add for extra leters with accent
					{
						this.charactersCount[5]++;//add to e
						this.charactersCount[31]++;//add to accents
					}
					else if(temporalChar =='í')//add for extra leters with accent
					{
						this.charactersCount[9]++;//add to e
						this.charactersCount[31]++;//add to accents
					}
					else if(temporalChar =='ó')//add for extra leters with accent
					{
						this.charactersCount[16]++;//add to o
						this.charactersCount[31]++;//add to accents
					}
					else if(temporalChar =='ú')//add for extra leters with accent
					{
